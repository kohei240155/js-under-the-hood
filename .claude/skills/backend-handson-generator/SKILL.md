...
0. **1ファイル ≒ 60分を目安にする（知識型のみ）** — 写経・実行・読解・予測検証を含めて、知識型の1ファイルは **60分以内に完走できる量** に抑える。プロジェクト型（システムデザイン / Capstone）はこの制約を適用しない。ボリュームが超えそうなら **複数ファイルに分割** する
1. **コードファースト** — 説明の前にまずコードを見せる。理論は後から補足する
2. **段階的に構築** — 最初から完成形を見せず、Step 1 → Step 2 → ... と積み上げる
3. **実務との対比** — 「実際のプロダクションではどうするか」「フレームワーク（Express / Prisma / etc.）が裏側でやっていること」の対比を要所に入れる（**全 Step ではなく** 知識型では1ファイル中 2〜3 箇所まで）
4. **なぜそうなるか** — コードの動作結果だけでなく「なぜこの設計判断をするのか」のメンタルモデルを言語化する
5. **失敗を体験させる** — 正しいコードだけでなく、わざと壊れるコード（セキュリティホール・パフォーマンス劣化・データ不整合）を書かせて「何が起きるか予測 → 実行 → 答え合わせ」のサイクルを回す（知識型ではファイル末尾の「発展課題」にまとめる）
...
> **注意**: システムデザイン問題（セクション 5）と Capstone（セクション 6）は **プロジェクト型** に分類される。1ファイル60分の制約は適用せず、要件整理・概算・設計・深掘りのフルセットを含める。
...
|---|---|---|---|---|
| **知識型（細）** | JWT認証 / インデックス最適化 / Token Bucket / CAP定理 | **3〜4 Step** | 200〜350 行 | 45〜60 分 |
| **知識型（中）** | 「インデックスとクエリ最適化」「認証認可」 | 4〜5 Step を **複数ファイルに分割** | 1ファイル300行以下 | 1ファイル60分 |
| **知識型（大／章）** | 「データベース設計とクエリ最適化」のような章見出し | 各サブトピックを **個別ファイル化**（章単位で1ファイルにしない） | 同上 | 同上 |
| **プロジェクト型** | システムデザイン問題（セクション 5）／ Capstone（セクション 6） | 制限なし（要件整理→概算→ハイレベル設計→深掘り 2〜3 箇所） | 制限なし | 60〜180 分 |

**判定基準:**
- `sections/backend/05-system-design/` または `sections/backend/06-capstone-project/` 配下、または「設計して」「を作る」「サービス」が含まれる → **プロジェクト型**
- それ以外（概念・API・データベース・パターンの理解）→ **知識型**
...
回答は **北米テック企業のバックエンド / フルスタック面接** を想定し、詳細版では英語で答える場合のキーワード・フレーズも併記する。
